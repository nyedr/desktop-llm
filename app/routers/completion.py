...
from app.dependencies.providers import get_agent, get_model_service
...
class GenerateCompletionForm(BaseModel):
    model: Optional[str] = Field(None, description="The model name to use for generation.")
    ...
    suffix: Optional[str] = Field(None, description="The text to append after the model response.")
    images: Optional[List[str]] = Field(None, description="A list of base64-encoded images for multimodal models.")
    task: Optional[str] = Field(None, description="Optional task type for prompt building.")

@router.get("/models")
async def list_models(
    request: Request,
    page: int = Query(1, ge=1, description="Page number."),
    limit: int = Query(10, ge=1, le=100, description="Number of models per page."),
    model_service: ModelService = Depends(get_model_service)
) -> Dict[str, Any]:
    """Get available models with pagination."""
    request_id = str(id(request))
    logger.debug(f"[{request_id}] Fetching models page {page} with limit {limit}")
...
async def generate_completion(
    ...
    request_id = str(id(request))
    logger.debug(f"[{request_id}] Generating completion for prompt: {form_data.prompt[:100]}...")
