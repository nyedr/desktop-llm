...
describe("File Operations", () => {
  ...
  describe("getFileStats", () => {
    it("should return correct file information", async () => {
      ...
      expect(new Date(stats.modified).getTime()).toBeGreaterThan(
        fiveMinutesAgo
      );
      expect(new Date(stats.accessed).getTime()).toBeGreaterThan(
        fiveMinutesAgo
      );
    });
  ...
  describe("searchFiles", () => {
    ...
    it("should find files matching pattern", async () => {
      ...
      expect(results.map((r) => path.basename(r))).toEqual(
        expect.arrayContaining(["file1.txt", "file3.txt"])
      );
    });
