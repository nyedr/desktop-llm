...
// Server state tracking with explicit types
...
function log(level: string, message: string, data?: any): void {
  ...
  const uptime = serverState.serverStartTime
    ? `[Uptime: ${Math.floor(
        (Date.now() - serverState.serverStartTime.getTime()) / 1000
      )}s]`
    : "";
  const prefix = `[Node.js ${
    process.pid
  }] [${level.toUpperCase()}] [${timestamp}] ${uptime}`;
 ...
 */
async function sendResult(id: string | number, result: any): Promise<void> {
 ...
 */
async function handlePing(request: JSONRPCRequest): Promise<void> {
...
// Add new types for file operations
...
let allowedDirectories: Set<string> = new Set();

// Add directory validation
async function isPathAllowed(filePath: string): Promise<boolean> {
  const absolutePath = path.resolve(filePath);
  return Array.from(serverState.allowedDirectories).some((dir) =>
...
// Define filesystem tools with improved error handling
interface Tool {
  description: string;
  parameters: {
    type: string;
...
async function searchDirectory(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      results.push(
        ...(await searchDirectory(fullPath, pattern, excludePatterns))
      );
 ...
 */
const SERVER_IMPLEMENTATION: Implementation = {
...
async function createServer(): Promise<Server> {
  ...
  server.setRequestHandler(MoveFileSchema, async (request, extra) => {
    try {
      ...
      if (
        !(await isPathAllowed(sourcePath)) ||
...
async function handleMessage(
  message: Request | Notification | Result
): Promise<void> {
  try {
    ...
    if ("method" in message && "id" in message) {
      ...
      try {
        ...
        switch (request.method) {
          case "read_file":
            await handleReadFile(request as ReadFileRequest);
            break;
          case "list_dir":
            await handleListDir(request as ListDirRequest);
            break;
          case "write_file":
            await handleWriteFile(request as WriteFileRequest);
            break;
          case "move_file":
            await handleMoveFile(request as MoveFileRequest);
            break;
          case "search_files":
            await handleSearchFiles(request as SearchFilesRequest);
            break;
...
type ReadFileRequest = JSONRPCRequest & {
  params: {
    path: string;
  };
};

type ListDirRequest = JSONRPCRequest & {
  params: {
    path: string;
  };
};

type WriteFileRequest = JSONRPCRequest & {
  params: {
    path: string;
    content: string;
  };
...
type MoveFileRequest = JSONRPCRequest & {
  params: {
    source: string;
    destination: string;
  };
...
type SearchFilesRequest = JSONRPCRequest & {
  params: {
    pattern: string;
    excludePatterns?: string[];
  };
...
async function handleReadFile(request: ReadFileRequest): Promise<void> {
  const validatedPath = await validatePath(request.params.path);
  log("debug", `Reading file: ${validatedPath}`);
...
async function handleListDir(request: ListDirRequest): Promise<void> {
  const validatedPath = await validatePath(request.params.path);
  log("debug", `Listing directory: ${validatedPath}`);
...
async function handleWriteFile(request: WriteFileRequest): Promise<void> {
  const validatedPath = await validatePath(request.params.path, false);
  if (!(await isPathAllowed(validatedPath))) {
    throw new Error("Path not in allowed directories");
  }
...
async function handleMoveFile(request: MoveFileRequest): Promise<void> {
  const sourcePath = await validatePath(request.params.source);
  const destPath = await validatePath(request.params.destination, false);
...
async function handleSearchFiles(request: SearchFilesRequest): Promise<void> {
  const results: string[] = [];
  for (const dir of serverState.allowedDirectories) {
    if (await isPathAllowed(dir)) {
      const matches = await searchDirectory(
...
if (process.env.ALLOWED_DIRECTORIES) {
  serverState.allowedDirectories = new Set(
